...
class _HistoryPageState extends ConsumerState<HistoryPage> {
  ...
  List<MedicationLogItem> _filterLogs(List<MedicationLogItem> logs) {
    final start = _dateOnly(_selectedDate.subtract(Duration(days: _rangeDays - 1)));
    final end = _dateOnly(_selectedDate);
    final q = _searchQuery.trim().toLowerCase();
    final out = logs.where((log) {
      final day = _dateOnly(log.scheduledDatetime.toLocal());
      final statusOk = _selectedStatus == 'all' || log.status == _selectedStatus;
      final inRange = !day.isBefore(start) && !day.isAfter(end);
      final queryOk = q.isEmpty || log.medicationName.toLowerCase().contains(q);
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            _SummaryCard(summary: summary, rangeDays: _rangeDays),
            ...
            for (final log in filtered)
              Card(
                ...
                child: ListTile(
                  ...
                  trailing: _StatusBadge(status: log.status, label: _labelForStatus(log.status)),
                ),
              ),
          ],
        ),
  ...
  void _showLogDetail(MedicationLogItem log) {
    final scheduled = DateFormat('dd/MM/yyyy HH:mm').format(log.scheduledDatetime.toLocal());
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) {
        ...
        Future<void> updateStatus(String status) async {
          ...
          updating = true;
          try {
            await ref.read(treatmentProvider.notifier).updateLogStatus(
                  profileId: profileId,
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Đã cập nhật trạng thái: ${_labelForStatus(status)}')),
              );
            }
          } catch (e) {
            if (mounted) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _StatusBadge(status: log.status, label: _labelForStatus(log.status)),
...
class _SummaryCard extends StatelessWidget {
  const _SummaryCard({required this.summary, required this.rangeDays});
