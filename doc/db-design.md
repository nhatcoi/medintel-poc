...
### Bảng `patient_agent_context` (ngữ cảnh agent dạng markdown)

Một dòng / `profile_id` [PK, FK → profiles, CASCADE]: bản **tri thức .md** đã render từ dữ liệu quan hệ (snapshot), để chat/agent **đọc text** thay vì ghép JSON snapshot mỗi lượt.

`content_markdown`: TEXT [NN] — nội dung markdown (chunk theo mục, không UUID trong phần hiển thị bệnh nhân).

`source`: VARCHAR(32) — ví dụ `snapshot_derived` | `manual` (sau này nếu cho sửa tay).

`format_version`: INT [NN] — bump khi đổi cấu trúc file → server làm mới bản cũ.

`created_at`, `updated_at`: TIMESTAMP

**Luồng:** server gọi `refresh_patient_agent_context_best_effort` sau **commit** các API đổi dữ liệu snapshot (profile, bệnh án, thuốc/lịch/log, memory, device-setup); thêm `POST .../agent-context/refresh` thủ công nếu cần. Chat với `include_patient_context_chunks` + `patient_context_use_stored_md` đọc cột này (tự refresh nếu trống/quá hạn/sai `format_version` — `patient_agent_context_max_age_seconds`).
