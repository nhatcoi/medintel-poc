...
class HealthHabit(Base, TimestampMixin):
    ...
    category_id: Mapped[uuid.UUID | None] = mapped_column(GUID, ForeignKey("habit_categories.category_id"), nullable=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    target_time: Mapped[time | None] = mapped_column(Time, nullable=True)
    status: Mapped[str | None] = mapped_column(String(64), nullable=True)

    reminders: Mapped[list[HabitReminder]] = relationship("HabitReminder", back_populates="habit", cascade="all, delete-orphan")
