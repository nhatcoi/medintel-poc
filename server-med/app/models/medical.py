"""§3 db-design: disease_categories, medical_records, treatment_periods."""
...
class MedicalRecord(Base, TimestampMixin):
    ...
    category_id: Mapped[uuid.UUID | None] = mapped_column(
        GUID, ForeignKey("disease_categories.category_id"), nullable=True, index=True
    )
    ...
    # Mở rộng pipeline OCR (không có trong bản gốc db-design tối thiểu)
...
class TreatmentPeriod(Base, TimestampMixin):
    ...
    record_id: Mapped[uuid.UUID] = mapped_column(
        GUID, ForeignKey("medical_records.record_id"), nullable=False, index=True
    )
    ...
    medical_record: Mapped[MedicalRecord] = relationship(
        "MedicalRecord", back_populates="treatment_periods"
    )
